...
ODatabase.prototype.open = function(username,passwd,callback){
	var self = this;	
	var auth = 'Basic ' + new Buffer(this.username + ':' + this.passwd).toString('base64');
	var header = {'Host': this.databasePath, 'Authorization': auth};
	var request = this.client.request('GET', '/connect/demo', header);
...
ODatabase.prototype.query = function(iQuery, iLimit, iFetchPlan,callback) {
	...
	var rquery = function(iQuery, iLimit, iFetchPlan) {
		...
		var auth = 'Basic ' + new Buffer(self.username + ':' + self.passwd).toString('base64');
		var header = {'Host': self.databasePath,'Authorization': auth};
		var url =  '/query/demo/sql/' + iQuery + iLimit + iFetchPlan; 
...
ODatabase.prototype.load = function(iRID, iFetchPlan,callback) {
	...
	var rload = function(){
		...
		var auth = 'Basic ' + new Buffer(self.username + ':' + self.passwd).toString('base64');
		var header = {'Host': self.databasePath,'Authorization': auth};
		var url =  '/document/demo/' + iRID + iFetchPlan;
		var request = self.client.request('GET', escape(url), header);
		request.on('response', function (response) {
			var body = '';
			response.on('data', function (chunk) {
...
ODatabase.prototype.save = function(obj,callback) {
	...
	var rsave = function(obj) {
		...
		var auth = 'Basic ' + new Buffer(self.username + ':' + self.passwd).toString('base64');
		var header = {'Host': self.databasePath,'Authorization': auth ,  'Content-Type': 'application/json', 'Content-Length': data.length};
		var url =  '/document/demo/' + rid;
